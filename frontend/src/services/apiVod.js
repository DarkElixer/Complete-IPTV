...
export const getSeriesOrMovie = async (
  { movieId = "", seasonId = "", episodeId = "", page = 1 },
  attempt = 1
) => {
  if (attempt >= 3) return;
  const res = await fetch(
    `${host}/vod/categories/series?movieId=${movieId}&seasonId=${seasonId}&episodeId=${episodeId}&page=${page}`,
    {
      method: "POST",
      body: JSON.stringify({ token: localStorage.token }),
      headers: {
  ...
  if (status === "fail") {
    ...
    return getSeriesOrMovie(
      { movieId, seasonId, episodeId, page },
      attempt + 1
    );
  }
  return data;
